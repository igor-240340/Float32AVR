...
SHFTMSB:    ROL A2                      ; A0=INT(NUM). (MSB МАНТИССЫ СОДЕРЖИТ LSB ИСТИННОЙ ЦЕЛОЙ ЧАСТИ - ВЫДВИГАЕМ ЕГО В A0.)
            ...
            ;
            ; ПРЕОБРАЗУЕТ ОДНОБАЙТОВОЕ НЕНУЛЕВО ЦЕЛОЕ ЧИСЛО В ЧИСЛО В ФОРМАТЕ ПЛАВАЮЩЕЙ ТОЧКИ.
            ; НОЛЬ НЕ БУДЕТ ОБРАБОТАН КОРРЕКТНО, ОЖИДАЕТСЯ ЦЕЛОЕ ЧИСЛО В ОТРЕЗКЕ [1,255].
            ;
            ; АРГУМЕНТЫ:
...
ITOF:       CLR A1                      ; МЛАДШИЕ БАЙТЫ МАНТИССЫ
            CLR A2                      ;
...
;
; ЗА ОСНОВУ ВЗЯТ АЛГОРИТМ ИЗ Z88DK С НЕКОТОРЫМИ МОДИФИКАЦИЯМИ.
...
; ОКРУГЛЕНИЕ ДЕСЯТИЧНОГО СТРОКОВОГО ПРЕДСТАВЛЕНИЯ НЕ ВЫПОЛНЯЕТСЯ.
;
; АРГУМЕНТЫ:
;   - NUM - ЧИСЛО. ОЖИДАЕТСЯ В РЕГИСТРАХ: R11, R10, R9, R8.
...
FTOA:       PUSH R12                    ; БЭКАПИМ PRECISION, Т.К. ОН НАХОДИТСЯ В ОДНОМ ИЗ ВХОДНЫХ РЕГИСТРОВ АРИФМЕТИЧЕСКИХ ОПЕРАЦИЙ.
...
GETINT:     PUSH A3                     ; БЭКАПИМ ПОСЛЕДНЕЕ ЗНАЧЕНИЕ NUM.
            ...
            ROL A3                      ;
            LDI R16,-127                ; EXP(NUM')-127.
            ADD R16,A3                  ; NUM'<1? (-127 ПРЕДСТАВЛЕН В ПСЕВДО ДОП. КОДЕ - В ПРЕДЕЛАХ ТОЛЬКО МЛАДШЕГО БАЙТА, ПОЭТОМУ ПРОВЕРКА ЗНАКА - НЕ ПО ФЛАГУ N, А ПО БИТУ ПЕРЕНОСА.)
            BRCS ASCIIDIG               ; НЕТ, ЦЕЛАЯ ЧАСТЬ НЕНУЛЕВАЯ, ФОРМИРУЕМ ЕЁ ЦИФРУ.
            LDI R16,0x30                ; ДА, ЛИБО РАЗРЯД С ПОРЯДКОМ DORDER РАВЕН НУЛЮ ЛИБО ВХОДНОЙ NUM<1 И РАЗРЯД ЕДИНИЦ ЧИСЛА NUM РАВЕН НУЛЮ, УСТАНАВЛИВАЕТ ЦИФРУ '0'.
            ST X+,R16                   ; *STR++='0'.
