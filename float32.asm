...
HALFWAY:    AND STATUS1,STATUS0         ; РАЗНОСТЬ НУЛЕВАЯ, ЕСЛИ ФЛАГ Z БЫЛ УСТАНОВЛЕН ДЛЯ КАЖДОГО БАЙТА.
            ...
            RJMP CHECKEXP1              ; НЕТ, RS=0b01000000 ИЛИ RS=0b00000000. МЛАДШАЯ ЧАСТЬ МЕНЬШЕ ULP/2, ПРОСТО ОТБРАСЫВАЕМ ЕЁ.
...
;
; В ОСНОВЕ ЛЕЖИТ АЛГОРИТМ ИЗ Z88DK, НО С НЕКОТОРЫМИ МОДИФИКАЦИЯМИ.
; [https://github.com/z88dk/z88dk/blob/aa60b9c9e4bab3318b9b10e919919058a4d3aaee/libsrc/math/cimpl/ftoa.c]
;
; ГЛАВНОЕ ДОПУЩЕНИЕ АЛГОРИТМА - МЫ ИГНОРИРУЕМ ТОТ ФАКТ, ЧТО ДЕСЯТИЧНОЕ ПРЕДСТАВЛЕНИЕ
...
;   - STR - УКАЗАТЕЛЬ НА ОБЛАСТЬ RAM, КУДА БУДЕТ ЗАПИСАНА ASCII-СТРОКА. ОЖИДАЕТСЯ В XH:XL.
            .DEF TEN0=R0                ; КОНСТАНТА ДЛЯ ХРАНЕНИЯ МНОЖИТЕЛЯ В ФОРМАТЕ FLOAT32.
            .DEF TEN1=R1                ;
            .DEF TEN2=R2                ;
            .DEF TEN3=R3                ;

            .DEF NUM0=R8                ; NUM - ЧИСЛО В FLOAT32, КОТОРОЕ НУЖНО ПРЕОБРАЗОВАТЬ В СТРОКУ.
            .DEF NUM1=R9                ;
            .DEF NUM2=R10               ;
            .DEF NUM3=R11               ;

            .DEF PREC=R12               ; PRECISION - КОЛИЧЕСТВО ДЕСЯТИЧНЫХ ЗНАКОВ ПОСЛЕ ТОЧКИ.

            .DEF PSTRL=R26              ; *STR - УКАЗАТЕЛЬ НА СТРОКУ,
            .DEF PSTRH=R27              ; ГДЕ БУДЕТ СФОРМИРОВАНО ДЕСЯТИЧНОЕ ПРЕДСТАВЛЕНИЕ NUM.

            .DEF NUMDWNN0=R4            ; ЗДЕСЬ ХРАНИТСЯ -(NUM/10.0F) НА ВРЕМЯ ВЫЧИСЛЕНИЯ ПОРЯДКА ORDER.
            .DEF NUMDWNN1=R5            ;
            .DEF NUMDWNN2=R6            ;
            .DEF NUMDWNN3=R7            ;

            .DEF INTCNT=R20             ; КОЛИЧЕСТВО ДЕСЯТИЧНЫХ ЗНАКОВ В ЦЕЛОЙ ЧАСТИ NUM, КОТОРЫЕ НУЖНО ИЗВЛЕЧЬ.

            .DEF STATACC=R21            ; АККУМУЛЯТОР РЕГИСТРА СТАТУСА.
...
FTOA:       LDI R16,0x00                ; TEN=10.0F.
            ...
            ;
            ; ПОНИЖЕНИЕ ДЕСЯТИЧНОГО ПОРЯДКА NUM, ЧТОБЫ ИЗБЕЖАТЬ ПЕРЕПОЛНЕНИЯ
            ; ПРИ ВЫЧИСЛЕНИИ ИСТИННОГО ЗНАЧЕНИЯ ЕГО ПОРЯДКА.
            PUSH PREC                   ; СОХРАНЯЕМ PRECISION, Т.К. ОН НАХОДИТСЯ В ОДНОМ ИЗ ВХОДНЫХ РЕГИСТРОВ АРИФМЕТИЧЕСКИХ ОПЕРАЦИЙ С FLOAT.
            PUSH NUM3                   ; СОХРАНЯЕМ ОРИГИНАЛЬНОЕ ЗНАЧЕНИЕ NUM ПЕРЕД ПЕРЕДАЧЕЙ ЕГО В FDIV32 КАК ДЕЛИМОГО.
            PUSH NUM2                   ; СОХРАНЯЕМ В LITTLE-ENDIAN ДЛЯ ЕДИНООБРАЗИЯ. 
            PUSH NUM1                   ;
            PUSH NUM0                   ;

            MOV R12,TEN0                ; ПЕРЕДАЕМ 10.0F В FDIV32 КАК ДЕЛИТЕЛЬ.
            MOV R13,TEN1                ;
            MOV R14,TEN2                ;
            MOV R15,TEN3                ;

            PUSH TEN3                   ; БЭКАПИМ TEN, ЧТОБЫ FDIV32 НЕ ПОВРЕДИЛ ЕГО.
            PUSH TEN2                   ;
            PUSH TEN1                   ;
            PUSH TEN0                   ;

            CALL FDIV32                 ; NUM=NUM/10.0F;

            POP TEN0                    ; ВОССТАНАВЛИВАЕМ TEN.
            POP TEN1                    ;
            POP TEN2                    ;
            POP TEN3                    ;

            ;
            ; ОПРЕДЕЛЕНИЕ ПОРЯДКА ORDER ЧИСЛА NUM.
            MOV NUMDWNN0,NUM0           ; NUMDWNN=-NUM.
            MOV NUMDWNN1,NUM1           ;
            MOV NUMDWNN2,NUM2           ;
            MOV NUMDWNN3,NUM3           ;
            COM NUMDWNN0                ;
            ...
            LDI R16,0x00                ; ORDER=1.0F.
            LDI R17,0x00                ; РАЗМЕЩАЕМ ORDER СРАЗУ КАК ПЕРВЫЙ МНОЖИТЕЛЬ ПОДПРОГРАММЫ FMUL32
            LDI R18,0x80                ; ПОСКОЛЬКУ ОН МОЖЕТ УМНОЖАТЬСЯ НЕСКОЛЬКО РАЗ ПОДРЯД.
            LDI R19,0x3F                ;
            MOV R8,R16                  ;
            MOV R9,R17                  ;
            MOV R10,R18                 ;
            ...
            LDI INTCNT,1                ; В СЛУЧАЕ NUM<1.0F НУЖНО ИЗВЛЕЧЬ ОДИН НУЛЕВОЙ РАЗРЯД ИЗ ЦЕЛОЙ ЧАСТИ NUM.

CALCORDER:  MOV R22,R8                  ; КОПИРУЕМ ORDER, ЧТОБЫ НЕ ИСКАЖАТЬ ОРИГИНАЛЬНОЕ ЗНАЧЕНИЕ.
            MOV R23,R9                  ;
            MOV R24,R10                 ;
            MOV R25,R11                 ;

                                        ;
                                        ; ВЫЧИСЛЕНИЕ РАЗНОСТИ ORDER-NUM.
                                        ;
                                        ; ЕСЛИ ORDER<=NUM, ЗНАЧИТ ИСТИННЫЙ ПОРЯДОК NUM БОЛЬШЕ ORDER.
                                        ; КАК ТОЛЬКО ORDER>NUM, ИСТИННЫЙ ПОРЯДОК НАЙДЕН.
                                        ;
                                        ; NOTE: ПОМНИМ, ЧТО ПОРЯДОК NUM ПРЕДВАРИТЕЛЬНО ПОНИЖЕН НА 10.
            LDI STATACC,0b00000010      ; МАСКА ДЛЯ ФЛАГА Z.
            ADD R22,NUMDWNN0            ;
            IN R16,SREG                 ; ИЗВЛЕКАЕМ ФЛАГ Z В STATACC.
            AND STATACC,R16             ;
            ADC R23,NUMDWNN1            ;
            IN R16,SREG                 ;
            AND STATACC,R16             ;
            ...
            AND STATACC,R16             ;
            ADC R25,NUMDWNN3            ;
            IN R16,SREG                 ; ИЗВЛЕКАЕМ ФЛАГИ N И Z. ЕСЛИ ОБА НУЛЕВЫЕ, ЗНАЧИТ ORDER>NUM И МЫ НАШЛИ ИСТИННЫЙ ПОРЯДОК NUM.
            LDI R17,0b00000100          ; ДОБАВЛЯЕМ В STATACC МАСКУ ДЛЯ ФЛАГА N.
            OR STATACC,R17              ;
            AND STATACC,R16             ; ORDER>NUM? (В R16 СЕЙЧАС ХРАНИТСЯ ПОСЛЕДНИЙ SREG.)
            BREQ RESTORENUM             ; ДА, ИСТИННЫЙ ПОРЯДОК NUM НАЙДЕН, ВОССТАНАВЛИВАЕМ ПОНИЖЕННЫЙ ПОРЯДОК NUM.
                                        ; НЕТ, ORDER<=NUM, ПОВЫШАЕМ ПОРЯДОК.
            MOV B0,TEN0                 ; B=10.0F. УСТАНАВЛИВАЕМ 10.0F КАК ВТОРОЙ МНОЖИТЕЛЬ ДЛЯ FMUL32.
            MOV B1,TEN1                 ; ORDER УЖЕ УСТАНОВЛЕН КАК ПЕРВЫЙ МНОЖИТЕЛЬ.
            MOV B2,TEN2                 ;
            MOV B3,TEN3                 ;

            PUSH TEN3                   ; БЭКАПИМ TEN, ЧТОБЫ FMUL32 НЕ ПОВРЕДИЛ ЕГО.
            PUSH TEN2                   ;
            PUSH TEN1                   ;
            PUSH TEN0                   ;

            PUSH NUMDWNN3               ; БЭКАПИМ NUMDWNN.
            PUSH NUMDWNN2               ;
            PUSH NUMDWNN1               ;
            PUSH NUMDWNN0               ;

            PUSH INTCNT                 ; БЭКАПИМ INTCNT.

            CALL FMUL32                 ; A=ORDER*10.0F.

            POP INTCNT                  ; ВОССТАНАВЛИВАЕМ INTCNT.

            POP NUMDWNN0                ; ВОССТАНАВЛИВАЕМ NUMDWNN.
            POP NUMDWNN1                ;
            POP NUMDWNN2                ;
            POP NUMDWNN3                ;
            ...
            INC INTCNT                  ; ПОРЯДОК NUM ВЫШЕ, ЧЕМ МЫ ПРЕДПОЛОЖИЛИ - НУЖНО БУДЕТ ИЗВЛЕЧЬ ДОПОЛНИТЕЛЬНЫЙ ДЕСЯТИЧНЫЙ РАЗРЯД ИЗ ЦЕЛОЙ ЧАСТИ.
            RJMP CALCORDER              ; ПОВТОРЯЕМ ПРОВЕРКУ ДЛЯ НОВОГО ЗНАЧЕНИЯ ПОРЯДКА.

            ;
            ; ВОССТАНОВЛЕНИЕ ИСХОДНОГО ЗНАЧЕНИЯ NUM ИЗ СТЕКА.
RESTORENUM: MOV B0,A0                   ; СМЕЩАЕМ ВЫЧИСЛЕННЫЙ ПОРЯДОК ORDER НА МЕСТО ДЕЛИТЕЛЯ.
            MOV B1,A1                   ;
            MOV B2,A2                   ;
            MOV B3,A3                   ;

            POP A0                      ; ВОССТАНАВЛИВАЕМ NUM НА МЕСТО ДЕЛИМОГО.
            POP A1                      ;
            POP A2                      ;
            POP A3                      ;

            ;
            ; ДЕСЯТИЧНАЯ НОРМАЛИЗАЦИЯ ЗНАЧЕНИЯ NUM В ОТРЕЗОК [0.0F,10.0F).
            PUSH INTCNT                 ; БЭКАПИМ СЧЕТЧИК ЦЕЛЫХ ДЕСЯТИЧНЫХ РАЗРЯДОВ.

            PUSH TEN3                   ; БЭКАПИМ КОНСТАНТУ 10.0F.
            PUSH TEN2                   ;
            PUSH TEN1                   ;
            PUSH TEN0                   ;

            PUSH A3                     ; БЭКАПИМ ИСХОДНОЕ (ДЕНОРМАЛИЗОВАННОЕ В ОБЩЕМ СЛУЧАЕ) ЗНАЧЕНИЕ NUM ПЕРЕД FDIV32.
            PUSH A2                     ; ПОСЛЕ ДЕСЯТИЧНОЙ НОРМАЛИЗАЦИИ И ИЗВЛЕЧЕНИЯ СТАРШЕГО РАЗРЯДА
            PUSH A1                     ; НАМ НУЖНО БУДЕТ СНОВА ИМЕТЬ ЕГО ДЕНОРМАЛИЗОВАННОЕ ЗНАЧЕНИЕ,
            PUSH A0                     ; НО УЖЕ БЕЗ СТАРШЕГО РАЗРЯДА.

            PUSH B3                     ; ORDER НЕ ИМЕЕТ ПОСТОЯННОГО ХРАНИЛИЩА,
            PUSH B2                     ; ПОЭТОМУ БЭКАПИМ ЕГО ПЕРЕД ВЫЗОВОМ FDIV32.
            PUSH B1                     ;
            PUSH B0                     ;

            CALL FDIV32                 ; A=NUM/ORDER.

            ;
            ; ДВОИЧНАЯ ДЕНОРМАЛИЗАЦИЯ МАНТИССЫ NUM.
            ROL A0                      ; РАСПАКОВЫВАЕМ A=NUM/ORDER.
            ROL A1                      ;
            ROL A2                      ;
            ROL A3                      ;
            SEC                         ;
            ...
            ROR A0                      ;
                                        ...
                                        ; ВОССТАНОВЛЕНИЕ ЗНАЧЕНИЯ ЭКСПОНЕНТЫ ИЗ КОДА СО СМЕЩЕНИЕМ.
                                        ;
                                        ; ЗНАЧЕНИЕ ЭКСПОНЕНТЫ В ЭТОЙ ВЕТКЕ ЛЕЖИТ В ОТРЕЗКЕ [127,127+3],
                                        ; ЗНАЧИТ РАЗНОСТЬ С 127 ВСЕГДА ДАСТ ПОЛОЖИТЕЛЬНОЕ ЗНАЧЕНИЕ,
                                        ; ПОЭТОМУ ДОСТАТОЧНО ДОП. КОДА В ПРЕДЕЛАХ БАЙТА.
            CLR R17                     ; R17 БУДЕТ СОДЕРЖАТЬ ЦЕЛУЮ ЧАСТЬ ДЕСЯТИЧНОГО ЗНАЧЕНИЯ NUM (NUM/ORDER).
            LDI R16,-127                ; A3=A3-127.
            ADD A3,R16                  ; ЭКСПОНЕНТА НУЛЕВАЯ, Т.Е. МАНТИССА УЖЕ ПРЕДСТАВЛЯЕТ ИСТИННОЕ ЗНАЧЕНИЕ?
            BREQ TOINT                  ; ДА, ПЕРЕХОДИМ К ИЗВЛЕЧЕНИЮ ЦЕЛОЙ ЧАСТИ ДЕСЯТИЧНОГО ЗНАЧЕНИЯ NUM.
            MOV R16,A3                  ; НЕТ, УСТАНАВЛИВАЕМ СЧЕТЧИК ЦИКЛА И РАСКРЫВАЕМ ЭКСПОНЕНТУ, ДЕНОРМАЛИЗУЯ МАНТИССУ.
DENORM:     ROL A0                      ; A << 1.
            ROL A1                      ;
            ROL A2                      ;
            ROL R17                     ;
            DEC R16                     ; МАНТИССА ДЕНОРМАЛИЗОВАНА ВЛЕВО НА ВЕЛИЧИНУ ЭКСПОНЕНТЫ?
            BREQ TOINT                  ; ДА, ПЕРЕХОДИМ К ИЗВЛЕЧЕНИЮ ЦЕЛОЙ ЧАСТИ ДЕСЯТИЧНОГО ЗНАЧЕНИЯ.
            RJMP DENORM                 ; НЕТ, ПРОДОЛЖАЕМ СДВИГ.

            ;
            ; ИЗВЛЕЧЕНИЕ ЦЕЛОЙ ЧАСТИ И ФОРМИРОВАНИЕ ЕЁ ASCII-КОДА.
            ;
            ; ASCII-КОД ЛЮБОЙ ЦИФРЫ СОДЕРЖИТ В МЛАДШЕМ ПОЛУБАЙТЕ ОБОЗНАЧАЕМОЕ ЕЮ ЧИСЛО В ДВОИЧНОМ ВИДЕ.
            ; А В СТАРШЕМ ПОЛУБАЙТЕ - ЧИСЛО 3 В ДВОИЧНОМ ВИДЕ.
TOINT:      ROL A2                      ; LSB ЦЕЛОЙ ЧАСТИ ЛЕЖИТ В MSB МАНТИССЫ,
            ROL R17                     ; ПОЭТОМУ ВЫДВИГАЕМ ЕГО В R17, ЧТОБЫ ЦЕЛАЯ ЧАСТЬ ПОЛНОСТЬЮ ЛЕЖАЛА В R17.
            LDI R16,0x30                ;
            ADD R16,R17                 ; DIGIT=DIGIT + '0'.
            ST X+,R16                   ; *STR++=DIGIT.

            ;
            ; ПРЕОБРАЗУЕМ ЦЕЛУЮ ЧАСТЬ ВО FLOAT.
            ; DIGIT=(FLOAT)DIGIT.
            CLR A0                      ; ЗАНУЛЯЕМ ДРОБНУЮ ЧАСТЬ ДЕСЯТИЧНО НОРМАЛИЗОВАННОГО NUM.
            CLR A1                      ;
            CLR A2                      ;
            ROR R17                     ; ВОССТАНАВЛИВАЕМ LSB ЦЕЛОЙ ЧАСТИ В MSB МАНТИССЫ.
            ROR A2                      ; МАНТИССА ДЕНОРМАЛИЗОВАНА НА ЭКСПОНЕНТУ И СОДЕРЖИТ (FLOAT)DIGIT=(FLOAT)((INT)NUM).

            MOV R16,A3                  ; ЕСЛИ В ЭКСПОНЕНТЕ УЖЕ 0, ТО ДВОЙНОЙ COM ДАСТ НОЛЬ.
            COM R16                     ; 
            COM R16                     ; МАНТИССА УЖЕ НОРМАЛИЗОВАНА?
            BREQ PACK1                  ; ДА, ПЕРЕХОДИМ К ЕЁ УПАКОВКЕ.
NORM1:      ROR R17                     ; НЕТ, НОРМАЛИЗУЕМ МАНТИССУ ВПРАВО НА ВЕЛИЧИНУ ИСХОДНОЙ ЭКСПОНЕНТЫ.
            ROR A2                      ;
            ROR A1                      ;
            ROR A0                      ;
            DEC R16                     ; МАНТИССА НОРМАЛИЗОВАНА?
            BRNE NORM1                  ; НЕТ, ПРОДОЛЖАЕМ НОРМАЛИЗАЦИЮ.
                                        ; ДА, ПЕРЕХОДИМ К УПАКОВКЕ.

            ;
            ; УПАКОВКА ЗНАЧЕНИЯ DIGIT=(FLOAT)DIGIT.
PACK1:      LDI R16,127                 ; ВОССТАНАВЛИВАЕМ ЗНАЧЕНИЕ ЭКСПОНЕНТЫ В КОДЕ СО СМЕЩЕНИЕМ.
            ADD A3,R16                  ;

            ROL A0                      ; УБИРАЕМ ЕДИНИЦУ У МАНТИССЫ.
            ROL A1                      ;
            ROL A2                      ;
            CLC                         ; ЗНАЧЕНИЕ ДОЛЖНО ОСТАТЬСЯ ПОЛОЖИТЕЛЬНЫМ.
