...
CALCORDER:  MOV R22,R8                  ; КОПИРУЕМ ORDER, ЧТОБЫ НЕ ИСКАЖАТЬ ОРИГИНАЛЬНОЕ ЗНАЧЕНИЕ.
            ...
            MOV R12,TEN0                ; УСТАНАВЛИВАЕМ 10.0F КАК ВТОРОЙ МНОЖИТЕЛЬ ДЛЯ FMUL32.
            MOV R13,TEN1                ; ORDER УЖЕ УСТАНОВЛЕН КАК ПЕРВЫЙ МНОЖИТЕЛЬ.
            MOV R14,TEN2                ;
            MOV R15,TEN3                ;
...
RESTORENUM: MOV R12,R8                  ; СМЕЩАЕМ ВЫЧИСЛЕННЫЙ ПОРЯДОК ORDER НА МЕСТО ДЕЛИТЕЛЯ.
            MOV R13,R9                  ;
            MOV R14,R10                 ;
            MOV R15,R11                 ;

            POP R8                      ; ВОССТАНАВЛИВАЕМ NUM НА МЕСТО ДЕЛИМОГО.
            POP R9                      ;
            POP R10                     ;
            POP R11                     ;
