...
FTOAN:      PUSH R12                    ; БЭКАПИМ PRECISION, Т.К. ОН НАХОДИТСЯ В ОДНОМ ИЗ ВХОДНЫХ РЕГИСТРОВ АРИФМЕТИЧЕСКИХ ОПЕРАЦИЙ.
...
;
; ЕСЛИ ЧИСЛО УЖЕ НОРМАЛИЗОВАНО ДЕСЯТИЧНО, ТО ПРОСТО ВЫПОЛНЯЕТСЯ КОНВЕРТАЦИЯ В СТРОКУ ЧЕРЕЗ FTOA
...
; В СТРОКУ ЧЕРЕЗ FTOA, НО С ОГРАНИЧЕННЫМ КОЛИЧЕСТВОМ ЗНАКОВ ПОСЛЕ ТОЧКИ, ТАКИМ, ЧТОБЫ ДЛИНА РЕЗУЛЬТИРУЮЩЕЙ
...
FTOAE:      PUSH MAXLEN                 ; БЭКАПИМ MAXLEN.
            ...
            ROL A3                      ;
            LDI R16,-127                ;
            ADD R16,A3                  ; ЭКСПОНЕНТА В КОДЕ СО СМЕЩЕНИЕМ ЛЕЖИТ В [1,126]?
            BRMI NORMLFTJMP             ; ДА, ЗНАЧИТ ИСТИННАЯ ЭКСПОНЕНТА ЛЕЖИТ В [-126,-1], А ЭТО ЗНАЧИТ, ЧТО NUM<1 И INT(NUM)=0 - НОРМАЛИЗУЕМ ВЛЕВО.
            
            POP A2                      ; НЕТ, NUM>=1, ЗНАЧИТ NUM ЛИБО УЖЕ НОРМАЛИЗОВАН, ЛИБО ДЕНОРМАЛИЗОВАН ВЛЕВО (ТОГДА НОРМАЛИЗУЕМ ВПРАВО).
            POP A3                      ; ВОССТАНАВЛИВАЕМ ОРИГИНАЛЬНОЕ ЗНАЧЕНИЕ NUM.
            PUSH A3                     ; БЭКАПИМ ОРИГИНАЛЬНОЕ ЗНАЧЕНИЕ NUM.
            PUSH A2                     ;
            PUSH A1                     ;
            PUSH A0                     ;
            ...
            LDI R16,0b10000000          ; A=NUM'=|NUM|.
            AND R16,A3                  ;
            EOR A3,R16                  ;

            PUSH EXP                    ;
            CALL FDIV32                 ; A=NUM'=FDIV32(NUM',10.0F). ПОНИЖАЕМ ПОРЯДОК NUM, ЧТОБЫ ИЗБЕЖАТЬ ПЕРЕПОЛНЕНИЯ ПРИ ВЫЧИСЛЕНИИ ЕГО ИСТИННОГО ПОРЯДКА.
            POP EXP                     ;

            MOV B0,A0                   ; B=A=NUM'.
            MOV B1,A1                   ; УДОБНО СРАЗУ РАСПОЛОЖИТЬ NUM' ТАК,
            MOV B2,A2                   ; ПОСКОЛЬКУ ORDER ПОСЛЕ ПЕРВОГО МАСШТАБИРОВАНИЯ ОКАЖЕТСЯ В A.
            MOV B3,A3                   ;

            LDI R16,ONE0                ; A=ORDER=1.0F.
            LDI R17,ONE1                ;
            LDI R18,ONE2                ;
            LDI R19,ONE3                ;
            MOV A0,R16                  ;
            ...
            ;
            ; ОПРЕДЕЛЕНИЕ ДЕСЯТИЧНОГО ПОРЯДКА ЧИСЛА NUM ДЛЯ ПОСЛЕДУЮЩЕЙ НОРМАЛИЗАЦИИ ВПРАВО.
NUMORDER:   PUSH A3                     ; БЭКАПИМ ТЕКУЩЕЕ ЗНАЧЕНИЕ ORDER, Т.К.
            PUSH A2                     ; ПЕРЕЗАПИШЕМ ЕГО ДОП. КОДОМ.
            PUSH A1                     ;
            PUSH A0                     ;
            ...
            ADD A0,B0                   ; A=NUM'-ORDER.
            ADC A1,B1                   ; ПЕРЕЗАПИСЫВАЕМ -ORDER, ЧТОБЫ СОХРАНИТЬ NUM'.
            ADC A2,B2                   ;
            ADC A3,B3                   ; ORDER>NUM'?
            BRMI NORMRGHT               ; ДА, МЫ НАШЛИ ИСТИННЫЙ ПОРЯДОК NUM, НОРМАЛИЗУЕМ ВПРАВО.
            
            POP A0                      ; НЕТ, ВОССТАНАВЛИВАЕМ МОДУЛЬ ORDER И ПОВЫШАЕМ ПОРЯДОК.
            POP A1                      ; A=ORDER.
            POP A2                      ;
            POP A3                      ;

            PUSH B3                     ; БЭКАПИМ NUM' ДЛЯ СЛЕДУЮЩЕЙ ИТЕРАЦИИ.
            PUSH B2                     ;
            PUSH B1                     ;
            PUSH B0                     ;
            ...
            PUSH EXP                    ;
            CALL FMUL32                 ; A=ORDER=FMUL(ORDER,10.0F).
            POP EXP                     ;
            INC EXP                     ; EXP++.
            POP B0                      ; B=NUM'. ВОССТАНАВЛИВАЕМ NUM'.
            POP B1                      ;
            POP B2                      ;
            POP B3                      ;
            RJMP NUMORDER               ;

NORMRGHT:   POP B0                      ; B=ORDER. ВОССТАНАВЛИВАЕМ МОДУЛЬ ORDER.
            POP B1                      ;
            POP B2                      ;
            POP B3                      ;
            ...
            PUSH EXP                    ;
            CALL FDIV32                 ; A=NUM=FDIV32(NUM,ORDER).
            POP EXP                     ;
            RJMP CONVMANT               ;

NORMLFT:    POP A2                      ; ВОССТАНАВЛИВАЕМ СТАРШИЕ ДВА БАЙТА NUM ВМЕСТО ТЕХ, КОТОРЫЕ "ПОСТРАДАЛИ" ПРИ РАСПАКОВКЕ ЭКСПОНЕНТЫ.
            POP A3                      ; A=NUM.
