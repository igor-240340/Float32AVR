...
SUM:        ADD R6,R7                   ; СКЛАДЫВАЕМ МАНТИССЫ A И B. У МАНТИССЫ A RGS-ЗОНА ВСЕГДА НУЛЕВАЯ, ПОЭТОМУ БИТ ПЕРЕНОСА НЕ ВОЗМОЖЕН.
            ADD R8,R12                  ;
            ADC R9,R13                  ;
            ADC R10,R14                 ;
                                        ; ПЕРЕПОЛНЕНИЕ ЕСТЬ?
...
ADDULP:     ADD R8,R16                  ; ПРИБАВЛЯЕМ ULP.
            ...
            ; ПРОВЕРКА ЭКСПОНЕНТЫ НА ПЕРЕПОЛНЕНИЕ/АНТИПЕРЕПОЛНЕНИЕ.
            ;
            ; ЭКСПОНЕНТА В КОДЕ СО СМЕЩЕНИЕМ ПРИНИМАЕТ ЗНАЧЕНИЯ В ОТРЕЗКЕ [-23,255].
            ...
            ; ЕСЛИ ЕСТЬ АНТИПЕРЕПОЛНЕНИЕ, ТО ЭКСПОНЕНТА ПРИНИМАЕТ ЗНАЧЕНИЯ В ОТРЕЗКЕ [-23,0] И ВЫЧИТАНИЕ ЕДИНИЦЫ ИЗ ЭКСПОНЕНТЫ ВСЕГДА ДАСТ ОТРИЦАТЕЛЬНОЕ ЗНАЧЕНИЕ.
