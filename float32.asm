...
FADD32:     ;
            ...
            ADC R7,R3                   ;
                                        ; |A|-|B|>=0?
            BRGE CALCSIGN               ; ДА, СВОП НЕ НУЖЕН. ПЕРЕХОДИМ К ВЫЧИСЛЕНИЮ ЗНАКА.
                                        ; НЕТ, ДЕЛАЕМ СВОП.
...
DIFF:       COM R7                      ; ВЫЧИСЛЯЕМ ПСЕВДО ДОП. КОД МАНТИССЫ B. ЭТО ДОПОЛНЕНИЕ ДО 2 ВМЕСТО 4.
            ...
            AND SREGACC,R16             ;
            ADD MANTA0,MANTB0           ; СКЛАДЫВАЕМ СЛЕДУЮЩУЮ ПАРУ БАЙТ МАНТИСС. У МАНТИССЫ A RGS-ЗОНА ВСЕГДА НУЛЕВАЯ, ПОЭТОМУ БИТ ПЕРЕНОСА ИЗ ПРЕДЫДУЩЕЙ ОПЕРАЦИИ НЕ ВОЗМОЖЕН НЕ ВОЗМОЖЕН.
            IN R16,SREG                 ;
            AND SREGACC,R16             ;
            ...
            IN R16,SREG                 ;
            AND SREGACC,R16             ; РЕЗУЛЬТАТ НУЛЕВОЙ?
            BRNE SETZERO                ; ДА, УСТАНАВЛИВАЕМ ПОЛОЖИТЕЛЬНЫЙ НОЛЬ. (ЕСЛИ ФЛАГ Z БЫЛ УСТАНОВЛЕН ДЛЯ КАЖДОЙ ПАРЫ БАЙТ, ТО SREGACC ОКАЖЕТСЯ НЕНУЛЕВЫМ.)
...
ADDULP:     ADD R8,R16                  ; ПРИБАВЛЯЕМ ULP.
            ...
            ; ЕСЛИ НЕТ АНТИПЕРЕПОЛНЕНИЯ, ТО ВЫЧИТАНИЕ ЕДИНИЦЫ ВСЕГДА ДАСТ ПОЛОЖИТЕЛЬНОЕ ЗНАЧЕНИЕ.
CHECKEXP2:  LDI R17,255                 ; ЗАПИСЫВАЕМ 255 В ДВА БАЙТА.
            ...
            IJMP                        ; ДА, ПЕРЕПОЛНЕНИЕ, ПРЫЖОК НА ОБРАБОТЧИК ОШИБОК, УКАЗАННЫЙ В РЕГИСТРЕ Z.
                                        ;
            NOP                         ; НЕТ, ПЕРЕХОДИМ К ПРОВЕРКЕ НА АНТИПЕРЕПОЛНЕНИЕ.
